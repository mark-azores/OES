                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="form-group col-12 col-md-12">
                            <input type="text" name="school_year" id="school_year" class="form-control" placeholder="School Year" required />
                        </div>
                    </div>
                </div>
...
<script>
    $(function () {
        ...
        $('#add_button').click(function(){
            ...
            $('#action').val('sy_add');
            $('#btn_action').val('sy_add');
        });
    
        $(document).on('click', '.status', function(){
            var id = $(this).attr('id');
            var status = $(this).data("status");
            var btn_action = 'sy_status';
            $.ajax({
                url:"action.php",
                method:"POST",
        ...
        $(document).on('click', '.update', function(){
            ...
            $.ajax({
                ...
                success:function(data)
                {
                    ...
                    $('#action').val('sy_update');
                    $('#btn_action').val('sy_update');
                },error:function()
                {
                    Toast.fire({
                        icon: 'error',
