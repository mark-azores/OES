...
$query .= "SELECT * FROM $FEES_TABLE WHERE  ";

if(isset($_POST["search"]["value"]))
{
	$query .= '(high_school LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR fee_category LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR fee_name LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR fee_amount LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR fee_type LIKE "%'.$_POST["search"]["value"].'%" ';
...
foreach($result as $row)
{
	...
	$sub_array[] = $row['high_school'];
	$sub_array[] = $row['fee_category'];
	$sub_array[] = $row['fee_name'];
	if ($row['fee_type'] == 'Add')
	{
		$sub_array[] = "(+)".number_format($row['fee_amount'], 2, '.', ',');
	}
	else
	{
		$sub_array[] = "(-)".number_format($row['fee_amount'], 2, '.', ',');
...
$output = array(
	...
	"recordsFiltered" 	=> 	get_total_all_records($connect, $FEES_TABLE),
	"data"				=>	$data
);
