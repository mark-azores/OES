...
FROM $ADMISSION_TABLE WHERE school_year = '" . $school_year . "' AND status IN ('Scheduled','Pending') $query");
if ($result) {
	foreach ($result as $row) {
		...
		if ($row['status'] == 'Pending') {
			...
			// after (data-grade_level="'.$row['grade_level'].'" name="status" )  ==> <!-- data-method="'.$row['payment_method'].'" data-sf="'.$row['sf_plan'].'"  data-me="'.$row['me_plan'].'" 
		...
		if (empty($row["button_status"])) {
			...
			// MODIFIED CODE OF THE ABOVE TO REMOVE REQUIREMENTS BUTTON IN ENROLLEES SHS
			$sub_array[] = $image . "<br><b>Admission No.</b>: " . $row['admission_no']
            . "<br><b>Grade Level</b>: " . $row['grade_level']
            . "<br><b>Strand</b>: " . $row['strand']
            . "<br><b>LRN</b>: " . $row['lrn']
