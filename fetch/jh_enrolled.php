...
if ($result) {
	foreach ($result as $row) {
		...
		//MODIFIED VERSION OF THE ABOVE COMMENT

		// Check if 'payment_method' key exists in $row array
		if (isset($row['payment_method'])) {
			// Check for 'Installment' payment method
			if ($row['payment_method'] == 'Installment') {
				// Your code for 'Installment' payment method
				$button = ' &nbsp; 
        <a class="btn btn-success update elevation-2 pr-3 pl-3" href="#" name="update" id="' . $row["id"] . '" style="border-radius: 20px;" data-toggle="tooltip" data-placement="top" title="Edit">
            <i class="fa fa-edit"></i> Edit
        </a>
        <a class="btn btn-success payment elevation-2 pr-3 pl-3" href="#" name="payment" id="' . $row["id"] . '" admission_no="' . $row["admission_no"] . '" status="' . $row["date_paid"] . '" status="' . $row["date_paid"] . '" sf="' . $row["sf_plan"] . '" me="' . $row["me_plan"] . '" style="border-radius: 20px;" data-toggle="tooltip" data-placement="top" title="View Payment">
            <i class="fa fa-file-invoice-dollar"></i> Payment
        </a>
        <a class="btn btn-success archive elevation-2 pr-3 pl-3" href="#" name="archive" id="' . $row["id"] . '" style="border-radius: 20px;" data-toggle="tooltip" data-placement="top" title="Archive">
            <i class="fa fa-archive"></i> Archive
        </a>';
			} else {
				// Your code for other payment methods
				$button = ' &nbsp; 
        <a class="btn btn-success update elevation-2 pr-3 pl-3" href="#" name="update" id="' . $row["id"] . '" style="border-radius: 20px;" data-toggle="tooltip" data-placement="top" title="Edit">
            <i class="fa fa-edit"></i> Edit
        </a>
        <a class="btn btn-success archive elevation-2 pr-3 pl-3" href="#" name="archive" id="' . $row["id"] . '" style="border-radius: 20px;" data-toggle="tooltip" data-placement="top" title="Archive">
			...
			// Handle case when 'payment_method' key is not set in $row array
			// You can assign a default value or handle it as per your requirement
		}
    ...
    <img class="img-fluid img-circle elevation-2" style="height: 50px; width: 50px; cursor: pointer;" id="user_img" src="' . $row["avatar"] . '" alt="' . $row["admission_no"] . '">
		...
		if (empty($row["button_status"])) {
			$sub_array[] = ' &nbsp; 
    <a class="btn btn-primary button_status elevation-2 pr-3 pl-3" href="#" data-status="Hide" name="button_status" id="' . $row["id"] . '" style="border-radius: 20px;" >
        <i class="fa fa-eye"></i> 
    </a> ' . $button;
			...
			$sub_array[] = ' &nbsp; 
    <a class="btn btn-primary button_status elevation-2 pr-3 pl-3" href="#" data-status="" name="button_status" id="' . $row["id"] . '" style="border-radius: 20px;" >
        <i class="fa fa-eye-slash"></i> 
    </a>' . $button;
			...
			$sub_array[] = $image . "<br><b>Grade Level</b>: " . $row['grade_level']
				...
				. "<br><b>LRN</b>: " . $row['lrn']
				. "<br><b>Status</b>: " . $row['student_status'] . ' <br>&nbsp; 
    <a class="btn btn-success requirements elevation-2 pr-3 pl-3" href="#" name="requirements" id="' . $row["admission_no"] . '" style="border-radius: 20px;" data-toggle="tooltip" data-placement="top" title="Requirements">
        <i class="fa fa-list"></i> Requirements
    </a>';
