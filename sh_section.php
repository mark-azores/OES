...
<script>
    $(function () {
        ...
        $('#add_button').click(function(){
            ...
            $('#action').val('shsection_add');
            $('#btn_action').val('shsection_add');
        });
        ...
        $(document).on('click', '.update', function(){
            var id = $(this).attr("id");
            var btn_action = 'shsection_fetch';
            $.ajax({
                url:"action.php",
                method:"POST",
                ...
                success:function(data)
                {
                    ...
                    $('#action').val('shsection_update');
                    $('#btn_action').val('shsection_update');
                },error:function()
                {
                    Toast.fire({
                        icon: 'error',
        ...
        $(document).on('click', '.status', function(){
            ...
            var btn_action = 'shsection_status';
            $.ajax({
                url:"action.php",
                method:"POST",
        ...
        $(document).on('click', '.view', function(){
            ...
            $.ajax({
                ...
                success:function(data)
                {
                    ...
                    $('#datatables_section').DataTable().destroy();
                    $('#datatables_section').html(data.section_table);  
                    $("#datatables_section").DataTable({ 
                        "initComplete": function () { 
                            $("#datatables_section").DataTable().buttons().container().appendTo( $('.col-sm-12:eq(0)', $("#datatables_section").DataTable().table().container() ) );   
                        },
                        "buttons": [ 
                            {
                                extend: 'print',
                                ...
                                action: function ( e, dt, node, config ) {
                                    // console.log(id, grade_level, section, adviser);
                                    var btn_action = 'print_student_list';
                                    $.ajax({
                                        url:"action.php",
                                        method:"POST",
                                        data:{id:id, btn_action:btn_action, grade_level:grade_level, section:section, adviser:adviser},
                                        dataType:"json",
                                        success:function(data)
                                        {
                    ...
                    $('#datatables_section').DataTable().draw();
                },error:function()
                {
                    Toast.fire({
                        icon: 'error',
